...
from config import COHERE_API_KEY, CONNECTOR_ID, CONNECTOR_API_KEY
import cohere
...
def create_chat(payload: ChatIn, session: Session = Depends(get_session)):
    """Handles chat requests between user and AI agent with RAG from Google Drive."""
    ...
    # --- 5️⃣ Prepare Cohere Chat API request ---
    ai_text = f"Cohere API not configured; echoing message: {payload.message}"
